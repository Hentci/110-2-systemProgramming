...
class SDN_stat_payload : public payload {
    ...
    public:
        ...
        const vector <pair<int, int>> getStat_info(){
            return stat_info;
        }

        void setStat_info(vector <pair<int, int>> _stat_info){
...
class SDN_stat_pkt_gen_event: public event {
    ...
    private:
        SDN_stat_pkt_gen_event (SDN_ctrl_pkt_gen_event &){}
...
// TODO
void stat_packet_event (unsigned int con_id, unsigned int id, vector <pair<int, int>> stat_info, unsigned int t = event::getCurTime(), string msg = "default"){
// void stat_packet_event (unsigned int dst, unsigned int t = 0, string msg = "default"){
    if ( id == BROCAST_ID || node::id_to_node(id) == nullptr ) {
        cerr << "id is incorrect" << endl; return;
    }
    // src -> switch, dst -> controller
    unsigned int src = id;
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p -> type() == "SDN_stat_packet"){
        ...
        SDN_stat_payload *l = nullptr;
        l = dynamic_cast<SDN_stat_payload*> (p -> getPayload());
        vector <pair<int, int>> stat_info = l -> getStat_info();
        int id = p -> getHeader() -> getSrcID();

        // cout << "SrcID: " << id << " " << msg << "\n";

        if(msg == "insStat"){
            for(auto [nei, cost]: stat_info){
                G[id].push_back(edge(false, nei, cost));
                // cout << id << " " << nei << " " << cost << "\n";
            }
            ...
            for(auto [nei, cost]: stat_info)
                G[id].push_back(edge(true, nei, cost));
            
        }

        // process p's information
        p -> getHeader() -> setNexID(con_id);
        send_handler(p);
    }
...
vector <int> dsts;
vector<map <int, vector <ar<int, 3>>>> updRound(2000);// [dst, [round, nodes...]]
map <ui, map <ui, ui>> table; // [id, dst, nxt]
vector <map <int, vector<int>>> updForest(mxN); //[dst, [i, [adj...]]]
/* HW4 */
vector <bool> insStatChecker(mxN), updStatChecker(mxN);
...
void SDN_controller::recv_handler(packet *p){
    ...
    if(p ->type() == "SDN_stat_packet"){
        vector <ll> d(mxN);
        vector <ui> par(mxN); // record the shortest path 
        ...
        if(msg == "insStat"){
            int id = p -> getHeader() -> getSrcID();
            insStatChecker[id] = true;
            
            bool ok = true;
            for(int i = 0;i < n;i++)
                if(!insStatChecker[i]) ok = false;
            if(ok){
                // cout << "???\n";
                // compute the routing tree
                // send update packet
                for(auto ele: dsts){
                    dijk(0, ele);
                    for(int i = 0;i < n;i++){
                        ...
                        ctrl_packet_event(con_id, i, ele, table[i][ele], event::getCurTime(), "InsRound");
            ...
            int id = p -> getHeader() -> getSrcID();
            updStatChecker[id] = true;

            bool ok = true;
            for(int i = 0;i < n;i++)
                if(!updStatChecker[i]) ok = false;

            if(ok){
                // cout <<"!!!\n";
                // compute the new routing tree
                // send update packet 1st round
                ...
                for(auto ele: dsts){
                    for(auto elee: updRound[ele][0]){
                        ctrl_packet_event(con_id, elee[0], elee[1], elee[2], event::getCurTime(), "UpdRound");
                    }
                }
            }
        }
...
map <int, vector <pair <int, int>>> insStats, updStats; // [nodeid, adj[nei, weight]] 
...
int main()
{
    ...
    for(int i = 0;i < m;i++){
        ...
        // hw4
        insStats[a].push_back({b, ow});
        insStats[b].push_back({a, ow});
        updStats[a].push_back({b, nw});
        updStats[b].push_back({a, nw});
    }
    ...
    for(ui id = 0;id < n;id++){
        ...
        // hw4 stat
        stat_packet_event(con_id, id, insStats[id], insTime, "insStat");
        stat_packet_event(con_id, id, updStats[id], updTime, "updStat");
    }
