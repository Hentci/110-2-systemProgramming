...
int main()
{
    ...
    for(int idx=0 ; idx<numLinks ; idx++){
        for(int col=0 ; col<5 ; col++){
            ...
            if(col == 1)
                ...
                // node::id_to_node(index)->add_phy_neighbor(neighbor);
                // node::id_to_node(neighbor)->add_phy_neighbor(index);
            }
            else if(col == 4){
                digit = tmp;
    ...
    // int length = 2*numDsts;

//     int Time[length],SrcID[length],DstID[length];
//     int count = 0,times = 2*numDsts;
//    while(times--){
//         cin >> Time[count] >> SrcID[count] >> DstID[count];
//         count++;
//     }
    ...
    for(int idx=0 ; idx<numNodes ; idx++){
        for(int i=0 ; i<numDsts ; i++){
            if(idx != Data.getDsts()[i]){
                ...
                // ctrl_packet_event(Data.getDsts()[i], InsTime, msg1);
                ...
                // ctrl_packet_event(Data.getDsts()[i], UpdTime, msg2);
                
            }
        }
    }
    // int reoccur[numNodes][numNodes];
    // int answer[numNodes][numNodes];
    // for(int idx=0 ; idx<numNodes ; idx++){
    //     // cout << idx << endl;
    //     for(int i=0 ; i<numNodes ; i++)
    //     reoccur[idx][i] = 0;
    ...
    //         int result2 = ShortestPath(idx,Data.getDsts()[i],2,Data);
    //         answer[idx][Data.getDsts()[i]] = result2;
    //         if(result1 == result2){
    ...
    //             // cout << idx << endl;
    //         times++;
    //         // cout << Data.getDsts()[i] << " " << answer[idx][Data.getDsts()[i]] << endl;
    //         string msg2 = to_string(Data.getDsts()[i]) + " " + to_string(answer[idx][Data.getDsts()[i]]);
    //         events.push_back(packetEvents(false, UpdTime, idx, msg2));
    //         }
    //     }
    //     }
    ...
    //     //cout << Time[i] << " " << SrcID[i] << " " << DstID[i] << endl;
    //     data_packet_event(SrcID[i],DstID[i],Time[i]);
    // }
