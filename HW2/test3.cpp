...
class SDN_switch: public node {
        ...
        set <ui> hi; // this is used for example; you can remove it when doing hw2
        bool ok;

    protected:
        SDN_switch() {} // it should not be used
        SDN_switch(SDN_switch&) {} // it should not be used
        SDN_switch(unsigned int _id): node(_id), ok(false) {} // this constructor cannot be directly called by users
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet" && ok ) { // the switch receives a packet from the other switch
        ...
        if(hi.count(p2 -> getPacketID())) return;
...
int main()
{
    ...
    sort(all(dataOutput), 
        [](ar <int, 3> A, ar <int, 3> B){ return A[2] == B[2] ? A[0] < B[0] : A[0] == B[0] ? A[1] < B[1] : 0; });
