...
class node {
    ...
    public:
        ...
        map <unsigned int, pair <int, int>> & getPhyNeighbors () { 
...
class SDN_switch: public node {
        ...
        map<unsigned int, map<unsigned int, unsigned int>> table; // output table <node id, destination id, ans>
...
const int mxn = 2e3;
vector <long long> d(mxn);
vector <unsigned int> par(mxn);
...
void SDN_switch::recv_handler (packet *p){
    ...
    /********** dijkstra ***********/
    auto dijk = [&](int isNew, unsigned int st) -> void{
        ...
        while(!pq.empty()){
            ...
            const map <unsigned int, pair <int, int>> &nblist = getPhyNeighbors();
            for(auto ele: )
        }
    };
...
int main()
{
    ...
    for (unsigned int id = 0; id < node::getNodeNum(); id ++){
        string msg = to_string(dst) + " " + to_string(dst); // (dst, next)
        // cout << msg << endl;
