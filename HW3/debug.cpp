...
class SDN_controller: public node {
        ...
        map <int, map <int, map <int, int>>> ack_status; // [dst, [round, [node, status]]]
    protected:
        SDN_controller() {} // it should not be used
        SDN_controller(SDN_controller&) {} // it should not be used
        SDN_controller(unsigned int _id): node(_id), hi(false), ack(false), updata_table(nodeNum,0){} // this constructor cannot be directly called by users
...
void SDN_controller::recv_handler (packet *p){
    if (p == nullptr) return ;
    //judge if all node send ack to controller
    /*bool find_ack = false;
    for(int i=0;i<nodeNum;i++){
        if(updata_table[i]%2 != 0){
            find_ack = true;
            ack = true;
        }
    }
    if(!find_ack) ack = false;*/

    //封包是ack
    if (p->type() == "SDN_ctrl_packet" && getNodeID() == p->getHeader()->getDstID()){
        //ack封包來了就直接收
        // updata_table[p->getHeader()->getSrcID()]++;
        // bool find_ack = false;
        // for(unsigned int i=0;i<nodeNum;i++){
        //     if(updata_table[i]%2 != 0){
        //         find_ack = true;
        ...
        // if(!ack){//表示這是最後一個ack封包，可以把之前存起來的non-ack封包送一送了
        //     for(auto&v:wait_packet){//ctrl_packet_event(con_id, dst, mat, act)
        //         ctrl_packet_event(getNodeID(), v.first, v.second.first, v.second.second);
        //     }
        //     wait_packet.clear();
        //     currRound++;
        //     for(auto destt: destID){
        //         for(int i = 0;i < nodeNum;i++)
        //             if(node_level[destt][i] == currRound)
        //                 ctrl_packet_event(nodeNum, i, destt, newG[i].Routing_Table[destt], event::getCurTime());
        //     }
    ...
    //封包是non-ack
        ...
        // //non-ack封包在可以傳non-ack的時間送來
        // SDN_ctrl_packet *p3 = nullptr;
        // p3 = dynamic_cast<SDN_ctrl_packet*> (p);
        // if(!ack)//直接送
        // {
        //     updata_table[p->getHeader()->getDstID()]++;
        //     p3 -> getHeader() -> setPreID(nodeNum);
        //     p3->getHeader()->setNexID ( p->getHeader()->getDstID() ) ;
        //     p3->getHeader()->setDstID ( p-> getHeader() -> getDstID());
        //     send_handler(p3);
        // }
        // //non-ack封包在不可以傳non-ack的時間送來
        // else//存起來
        // {
        ...
        //     wait_packet.push_back(make_pair(dst, make_pair(Mat, Act)));
        // }
...
int main()
{
    ...
    for(unsigned int k=0;k<destNum;k++)
    {
        ...
        // vector<int> node_level(nodeNum, 0);
        ...
        for(unsigned int i=0;i<nodeNum;i++)
        {
            ...
            // cout << "nxtnode " << newG[i].Routing_Table[destID[k]]<< "\n";
        }

        //delete the same table // 加這個會錯QQ
        // for(unsigned int i=0;i<nodeNum;i++)
        // {
        //     if(oldG[i].Routing_Table.find(destID[k])!=oldG[i].Routing_Table.end())//destID[k] exist in table
        //         if(oldG[i].Routing_Table[destID[k]]==newG[i].Routing_Table[destID[k]])
        //             newG[i].Routing_Table.erase(destID[k]);
        // }
        //send update ctrl packet for UpdTime
        // for(unsigned int i=1;i<nodeNum;i++)
        //     for(unsigned int j=0;j<nodeNum;j++)
        //         if(node_level[j]==i)//從level  1~nodeNum-1慢慢送
        //             for(auto&v:newG[j].Routing_Table)
        //                 ctrl_packet_event(con_id, j, v.first, v.second, UpdTime);
        //

        // 一開始只送level1,剩下在controller慢慢送
        for(int i = 0;i < nodeNum;i++)
