...
class SDN_controller: public node {
        ...
        map <int, map <int, map <int, int>>> upd_ctrl_status; // [dst, [round, [node, status]]]
...
// TODO
int upd_curr_Round = 0;
// ctrl_status [dst, [node, status]]
vector <int> dsts;
// vector<map <int, vector <ar<int, 3>>>> updRound(2000), insRound(2000);// [dst, [round, nodes...]]
map <pair<int, int>, vector<pair<int, int>> > updRound; // map <{dstID, round}, {nodeID, nxtID}>
void SDN_controller::recv_handler(packet *p){
    ...
    if(p -> type() == "SDN_ctrl_packet" ){
        ...
        if(dstID == con_id){ // ack 1 -> 2
            ...
            /** check **/
            if(msg == "InsRound"){
                /* Don't need to build the initial tree QQ */
            }
                ...
                for(auto ele: dsts){
                    for(auto roundNode: updRound[{ele, upd_curr_Round}]){
                        if(upd_ctrl_status[ele][upd_curr_Round][roundNode.first] != 2)
                            ok = false;
                    }
                }
                if(ok){
                    ...
                    for(auto ele: dsts){
                        for(auto roundNode: updRound[{ele, upd_curr_Round}]){
                            ctrl_packet_event(con_id, roundNode.first, ele, roundNode.second, event::getCurTime(), "UpdRound");
                        }
                        // for(int i = 0;i < 5;i++){
            ...
            // cout << msg << "\n";
            if(msg == "UpdRound") upd_ctrl_status[mat][upd_curr_Round][dstID] = 1;
...
int main()
{
    ...
    // one dst one rule
    // vector <map <int, set<int>>> insForest(2000), updForest(2000); //[dst, [i, [adj...]]]

    // vector <int> insDeep(2000, 0);
    // FIXME
    ...
    // };
    // auto updProcRule = [&](vector<map<int, vector<int>>> forest, vector<map<int, vector<ar<int, 3>>>> &round) -> void{
    //     vector <bool> bfs_ok(n);
    //     vector <int> bfs_par(n);
    //     for(auto ele: dsts){
    //         queue <ar<int, 2>> q; //[node, level]
    ...
    map <pair<int, int>, set<int>> forest[2];
    ...
    for(auto ele: dsts){
        ...
        for(int i = 0;i < n;i++){
            if(i == ele) continue;
            table[i][ele] = par[i];
            // insForest[ele][i].insert(table[i][ele]);
            // insForest[ele][table[i][ele]].insert(i);
            forest[0][{(int)table[i][ele], ele}].insert(i);
            forest[0][{i, ele}].insert((int)table[i][ele]);
    ...
    for(auto ele: dsts){
        ...
        for(int i = 0;i < n;i++){
            ...
            // }
            table[i][ele] = par[i];
            // updForest[ele][i].insert(table[i][ele]);
            // updForest[ele][table[i][ele]].insert(i);
            forest[1][{(int)table[ele][i], ele}].insert(i);
            forest[1][{i, ele}].insert((int)table[ele][i]);
            // ctrl_packet_event(con_id, i, ele.id, table[i][ele.id], updTime);
            // string msg = to_string(ele.id) + " " + to_string(table[i][ele.id]);
            // cout << msg << "\n";
        }
    }
    auto initRec = [&](auto &&f, int lv, int dst, vector <int> &rnd, int currRnd, vector <pair<int, int>> &initLeaves) -> void{
        for(auto ele: forest[0][{lv, dst}]){
            if(forest[0][{ele, dst}].empty()){
                rnd[ele] = currRnd;
                initLeaves.push_back({lv, ele});
                continue;
            }

            f(f, ele, dst, rnd, currRnd, initLeaves);
        }
    };

    auto updRec = [&](auto &&f, int lv, int dst, vector <int> &rnd) -> void{
        for(auto ele: forest[1][{lv, dst}]){
            chmin(rnd[ele], rnd[lv] + 1);
            f(f, ele, dst, rnd);
        }
    };

    vector <int> rnd;
    vector <pair<int, int>> initLeaves;
    auto halfRule = [&]() -> void{
        for(auto ele: dsts){
            rnd.clear();
            for(int i = 0;i <= n;i++)
            ...
            int currRnd = 1;
            while(!forest[0][{ele, ele}].empty()){
                initLeaves.clear();
                initRec(initRec, ele, ele, rnd, currRnd, initLeaves);
                for(auto leaf: initLeaves)
                    forest[0][{leaf.first, ele}].erase(leaf.second);
                currRnd++;
            }

            updRec(updRec, ele, ele, rnd);

            for(int i = 0;i < n;i++){
                if(i == ele) continue;
                updRound[{ele, rnd[i] - 1}].push_back({i, table[ele][i]});
            }
        }
    };
    halfRule();

    // updProcRule(updForest, updRound);
    /** debug **/
    // for(int i = 0;i < n;i++){
    //     cout << i << "dst: \n";
    ...
    for(auto ele: dsts){
        for(auto elee: updRound[{ele, 0}]){
            ...
            ctrl_packet_event(con_id, elee.first, ele, elee.second, updTime, "UpdRound");
        }
    }
