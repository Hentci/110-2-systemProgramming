...
vector <int> dsts;
vector<map <int, vector <ar<int, 3>>>> updRound(2000), insRound(2000), newUpdRound(2000);// [dst, [round, nodes...]]
    ...
    if(p -> type() == "SDN_ctrl_packet" ){
        ...
        if(dstID == con_id){ // ack 1 -> 2
            ...
            if(msg == "InsRound"){
                ...
                bool ok = true;
                for(auto ele: dsts){
                    for(auto roundNode: newUpdRound[ele][upd_curr_Round]){
...
int main()
{
    ...
    vector <vector <int>> nodeRnd(2000, vector <int> (2000 + 5, 0x3f3f3f));
    for(auto dstt: dsts){
    ...
    //     cout<< "DST: " << ele << "\n";
    //     for(int i = 0;i < n;i++){
    //         cout << "node: " << i << " round: " << nodeRnd[ele][i] << "\n";
    //     }
    // }
    ...
    for(auto ele: dsts){
        for(int i = 0;i < n;i++){
            if(i == ele) continue;
            newUpdRound[ele][nodeRnd[ele][i]].push_back({i, ele, (int)table[i][ele]});
        }
    }
    ...
    // for(auto dsttt: dsts){
    //     cout << "dst: " << dsttt << "\n";
    //     for(auto ele: newUpdRound[dsttt]){
    //         cout << ele.first << "(round): \n";
    //         for(auto ndd: ele.second){
    //             cout << ndd[0] << " ";
    //         }
