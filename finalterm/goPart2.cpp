...
int n, nodeCost, insTime, updTime, 
simDuration, OLDsrc, OLDdst, OLDm, conid, m,
statChecker;

// table
map <int, int> insTable; // [id, nxt]

vector <map <ui, ui>> updStat(2000);
vector <vector <ar<int, 2>>> G(2000);

vector <pair<int, int>> red, white, black; // id, nxtID
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        ...
        if(dst == getNodeID() || table.count(dst) == 0) return;
        ...
        p3 -> getHeader() -> setDstID(conid);
        p3 -> getHeader() -> setNexID(conid);
        ...
        map<ui, ui> stat_info = l4 -> getLinkWeight();
        int id = p4 -> getHeader() -> getSrcID();
        for(auto [nei, wei]: stat_info){
            G[id].push_back({(int)nei, (int)wei});
        }
...
map <int, bool> redChecker, whiteChecker, blackChecker;
// auto whiteITR = white.begin();
vector <pair<int, int>> :: iterator whiteITR;
pair <int, int> lastTWO;
bool needlastTWO, whiteOK, blackOK, lasTWOOK;
void SDN_controller::recv_handler (packet *p){
    ...
    if(p->type() == "SDN_stat_packet"){ // the controller receives a control packet
        ...
        l4 = dynamic_cast<SDN_stat_payload *> (p4->getPayload());
        statChecker++;
        if(statChecker == n){   // stat packet over
            // first round
            // cout <<"assssssssssssssssss\n";
            for(auto [a, b]: red)
                ctrl_new_packet_event(conid, a, OLDdst, b, event::getCurTime(), "redFirstRound");

            if(red.empty()){
                for(auto ele: white){
                    if(needlastTWO){
                        if(ele == lastTWO) continue;
                    }
                    ctrl_upd_packet_event(conid, ele.first, OLDdst, ele.second, event::getCurTime(), "white");
                    whiteOK = true;
                }
            }
            else if(red.empty() && white.empty()){
                for(auto [a, b]: black)
                    ctrl_del_packet_event(conid, a, OLDdst, b, event::getCurTime(), "blackLastRound");
            }
        }
    }
    else{
        ui mat, act, dst, pre, ndID = getNodeID();
        string msg;
        if(p -> type() == "SDN_ctrl_new_packet"){
            ...
            mat = l3->getMatID(); 
            act = l3->getActID(); 
            dst = p3 -> getHeader() -> getDstID();
            pre = p3 -> getHeader() -> getPreID();
            msg = l3 -> getMsg();
        }
        ...
        if(dst != ndID){ 
            // to switch
            p -> getHeader() -> setNexID(dst);
            p -> getHeader() -> setPreID(ndID);
            send_handler(p);
        }
        else if(msg != "install OLD path"){
            // from switch && update
            bool redOK = true;
            if(p -> type() == "SDN_ctrl_new_packet"){
                redChecker[pre] = true;
                // cout << "isRED from switch\n";
                for(auto ele: red)
                    if(!redChecker[ele.first]) redOK = false;

                if(redOK){
                    // cout << "okok\n";
                    // launch first update round
                    // if(whiteITR != white.end()){
                    //     ctrl_upd_packet_event(conid, whiteITR -> first, OLDdst, whiteITR -> second, event::getCurTime(), "white");
                    //     whiteITR++;
                    // }
                ...
                if(blackOK){
                    if(needlastTWO && !lasTWOOK){
                        ctrl_upd_packet_event(conid, lastTWO.first, OLDdst, lastTWO.second, event::getCurTime(), "white");
                        lasTWOOK = true;
                    }

                    for(auto [a, b]: black){
                        ctrl_del_packet_event(conid, a, OLDdst, b, event::getCurTime(), "black");
                    }
                }
            }
...
vector<int> v, pre[2005], v1[2005];
vector<pii> v2[2005];
vector<vector<int>> all_shortest;
long long dis[2005], ans = 0;
void dfs(int x){
    if(x == OLDsrc){
        vector<int> u = v;
        reverse(u.begin(), u.end());
        all_shortest.push_back(u);
        return;
    }
    for(auto i : pre[x]){
        v.push_back(i);
...
int main(){
    ...
    vector<int> ans_vec;
    set<pii> old_edge;// old edge
    cin >> n >> nodeCost >> insTime >> updTime >> simDuration >> OLDsrc >> OLDdst >> OLDm;

    for(ui id = 0;id < n;id++)
        node::node_generator::generate("SDN_switch", id);

    conid = node::getNodeNum();
    node::node_generator::generate("SDN_controller", conid);
    for(int id = 0;id < n;id++){
        node::id_to_node(id) -> add_phy_neighbor(conid);
        node::id_to_node(conid) -> add_phy_neighbor(id);    
    }

    // table -> [id, [dst, nxt]]
    int linkID, from, to;
    ans_vec.push_back(OLDsrc);
    for(int i = 0;i < OLDm;i++){
        cin >> linkID >> from >> to;
        insTable[from] = to;
        ans_vec.push_back(to);
        old_edge.insert({from,to});
        v1[from].push_back(to);
        ctrl_new_packet_event(conid, from, OLDdst, to, insTime, "install OLD path");
    }
    ...
    for(int i = 0;i < m;i++){
        ...
        if(old_edge.find({a, b}) != old_edge.end() || old_edge.find({b, a}) != old_edge.end()) ans += w;
        v2[a].push_back({b, w});
        v2[b].push_back({a, w});
        updStat[a][b] = w;
        updStat[b][a] = w;
    ...
    // DIJKSTRA FIND ALL SHORTEST PATH
    ...
    int path_cost = dis[OLDdst];
    // ALL SHORTEST PATH
    v.push_back(OLDdst);
    dfs(OLDdst);
    for(auto vec : all_shortest){
        set<pii> new_edge;
        set<int> node_change; // count how many node change 
        for(int i = 0; i < vec.size() - 1; i++){
            new_edge.insert({vec[i], vec[i + 1]});
            if(old_edge.find({vec[i], vec[i + 1]}) == old_edge.end()){
                node_change.insert(vec[i]);
            }
        }
        for(auto i : old_edge){
            if(new_edge.find(i) == new_edge.end()){
                node_change.insert(i.f);
            }
        }
        int size = (node_change.find(OLDdst) != node_change.end()) ? node_change.size() - 1 : node_change.size();
        // cout << "_COST: " << path_cost + size * nodeCost << '\n';
        // cout << path_cost << '\n';
        // cout << size << '\n';
        if(path_cost + size * nodeCost < ans){
            ans = path_cost + size * nodeCost;
            ans_vec.clear();
    ...
    for(int i = 0;i < n;i++){
        stat_packet_event(i, conid, updStat[i], updTime, "updStat");
    }
    
    // red -> old(no), new(yes) ADD first round
    // black -> old(yes), new(no) DELETE last round
    // white -> always(yes) UPDATE 
    ...
    for(int i = 0;i < (int)ans_vec.size() - 1;i++){
        if(insTable.find(ans_vec[i]) != insTable.end())
            white.push_back({ans_vec[i], ans_vec[i + 1]});
        else
            red.push_back({ans_vec[i], ans_vec[i + 1]});
        
        newPath[ans_vec[i]] = ans_vec[i + 1];
    }

    for(auto it = insTable.begin();it != insTable.end();it++){
        if(newPath.find(it -> first) == newPath.end()){
            black.push_back({it -> first, it -> second});
        }
    }

    if(white.size() > 2) needlastTWO = true;

    lastTWO = white[(int)white.size() - 2];
    

    // reverse(all(white));

    whiteITR = white.begin();


    // cout << "red: \n";
    // for(auto [a, b]: red) cout << a << " " << b << "\n";
    // cout << "white: \n";
    // for(auto [a, b]: white) cout << a << " " << b << "\n";
    // cout << "black: \n";
    // for(auto [a, b]: black) cout << a << " " << b << "\n";


    // cout << lastTWO.first << " " << lastTWO.second << '\n';
